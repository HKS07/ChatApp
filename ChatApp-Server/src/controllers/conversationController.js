...
export const createConversation = async (req, res) => {
  ...
  try {
    ...
    if (!primaryUser || !secondaryUser)
      return res
        .status(404)
        .json({ message: "Either or both the users doens't exists." });
    ...
    return res
      .status(500)
      .json({ message: "Error while createConversation", error: error });
  }
};
...
export const getConversationById = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Error while getConversationById", error: error });
  }
};
